...
class NotificationService {
  ...
  async getPendingNotifications(): Promise<NotificationEvent[]> {
    ...
    const pendingNotifications = await db
      ...
      .where(
        and(eq(notificationEvents.sent, false), lte(notificationEvents.scheduledAt, new Date()))
      )
  ...
  async createDefaultUserNotificationSettings(userId: string): Promise<UserNotificationSettings> {
  ...
  async updateUserNotificationSettings(
    userId: string,
    settings: Partial<Omit<UserNotificationSettings, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>
    ...
    if (!existingSettings) {
      // 기존 설정이 없으면 새로 생성
      const newSettings = await this.createDefaultUserNotificationSettings(userId)
  ...
  async scheduleNotificationForTodo(todoId: string, userId: string, dueDate: Date): Promise<void> {
    ...
    if (!userSettings) {
      console.log(`No notification settings found for user ${userId}, creating defaults`)
      await this.createDefaultUserNotificationSettings(userId)
    }

    const now = new Date()
    const notifications: Array<Omit<NewNotificationEvent, 'id' | 'createdAt' | 'sent'>> = []
    ...
    for (const reminderTime of reminderTimes) {
      ...
      const reminderTime_date = new Date(dueDate.getTime() - minutesBefore * 60 * 1000)
    ...
    await db
      ...
      .where(and(eq(notificationEvents.todoId, todoId), eq(notificationEvents.sent, false)))
  ...
  async deleteNotificationsForTodo(todoId: string): Promise<void> {
    ...
    await db.delete(notificationEvents).where(eq(notificationEvents.todoId, todoId))
  }
