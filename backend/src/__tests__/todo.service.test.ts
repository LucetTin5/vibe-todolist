...
describe('TodoService', () => {
  ...
  describe('확장된 필터링 기능', () => {
    ...
    test('태그로 검색할 수 있다', async () => {
      const projectResult = await service.getTodos({ page: 1, limit: 10 }, 'all', 'project')

      expect(projectResult.todos).toHaveLength(1)
      expect(projectResult.todos[0].tags).toContain('project')

      const urgentResult = await service.getTodos({ page: 1, limit: 10 }, 'all', 'urgent')
    ...
    test('제목과 설명에서 검색할 수 있다', async () => {
      const titleResult = await service.getTodos({ page: 1, limit: 10 }, 'all', 'Work Task')
      ...
      expect(descriptionResult.todos[0].description).toContain('Doctor appointment')
    })
  ...
  describe('정렬 기능', () => {
    ...
    test('마감일로 정렬할 수 있다', async () => {
      ...
      const dueDates = ascResult.todos.map((todo) => todo.dueDate).filter(Boolean)
  ...
  describe('에러 처리', () => {
    ...
    test('존재하지 않는 Todo 처리', async () => {
      ...
      await expect(service.updateTodo('nonexistent', { title: '새 제목' })).rejects.toThrow(
        'Todo를 찾을 수 없습니다'
      )
