...
describe("TodoService", () => {
  ...
  describe("Todo 조회", () => {
    ...
    test("페이지 번호가 0 이하면 에러가 발생한다", async () => {
      await expect(
        service.getTodos({ page: 0, limit: 10 }, "all")
      ).rejects.toThrow("페이지 번호는 1 이상이어야 합니다");
    });

    test("페이지 크기가 0 이하면 에러가 발생한다", async () => {
      await expect(
        service.getTodos({ page: 1, limit: 0 }, "all")
      ).rejects.toThrow("페이지 크기는 1 이상이어야 합니다");
    });

    test("페이지 크기가 100을 초과하면 에러가 발생한다", async () => {
      await expect(
        service.getTodos({ page: 1, limit: 101 }, "all")
      ).rejects.toThrow("페이지 크기는 100 이하여야 합니다");
    });
  });
  ...
  describe("Todo 검색", () => {
    ...
    test("빈 검색어는 무시된다", async () => {
      const resultWithEmpty = await service.getTodos(
        { page: 1, limit: 10 },
        "all",
        ""
      );

      const resultWithoutSearch = await service.getTodos(
        { page: 1, limit: 10 },
        "all"
      );
  ...
  describe("Todo 삭제", () => {
    test("기존 Todo를 삭제할 수 있다", async () => {
      ...
      await expect(service.getTodoById(created.id)).rejects.toThrow(
        "Todo를 찾을 수 없습니다"
      );
    });
