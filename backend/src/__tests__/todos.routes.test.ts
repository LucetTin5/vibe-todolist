...
describe("Todo API Routes", () => {
  ...
  describe("GET /api/todos - Enhanced 필터링", () => {
    ...
    test("정렬 기능이 동작한다", async () => {
      const titleAscResponse = await app.request(
      ...
      const priorityDescResponse = await app.request(
        "/api/todos?sortBy=priority&sortOrder=desc"
      );
      expect(priorityDescResponse.status).toBe(200);

      const priorityResult = await priorityDescResponse.json();
      const priorities = priorityResult.todos.map(
  ...
  describe("PATCH /api/todos/:id/toggle - 상태 토글", () => {
    test("Todo의 완료 상태를 토글할 수 있다", async () => {
      ...
      // 다시 토글
      const toggleAgainResponse = await app.request(
        `/api/todos/${created.id}/toggle`,
        {
          method: "PATCH",
        }
